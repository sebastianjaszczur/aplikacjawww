...
( function() {
	CKEDITOR.dialog.add( 'link', function( editor ) {
		var plugin = CKEDITOR.plugins.linklocal;
		...
		var targetChanged = function() {
				...
				switch ( value ) {
					case 'frame':
						targetName.setLabel( editor.lang.linklocal.targetFrameName );
						targetName.getElement().show();
						break;
					case 'popup':
						popupFeatures.show();
						targetName.setLabel( editor.lang.linklocal.targetPopupName );
						targetName.getElement().show();
						break;
					default:
		...
		var linkTypeChanged = function() {
				var dialog = this.getDialog(),
					partIds = [ 'urlOptions', 'localOptions', 'anchorOptions', 'emailOptions' ],
		...
		var commonLang = editor.lang.common,
			linkLang = editor.lang.linklocal,
			anchors;
						...
						},
						{
							...
							onKeyUp: function() {
								...
								var protocolCmb = this.getDialog().getContentElement( 'info', 'protocol' ),
									...
									urlOnChangeTestLocal = /^(\/)/i;

								var localprotocol = urlOnChangeTestLocal.exec( url );
...
//----------------------------------------------------------------------
				{
					...
					children: [ {
						type: 'text',
						id: 'localurl',
						label: linkLang.localLink,
						required: true,
						validate: function() {
							var dialog = this.getDialog();

							if ( dialog.getContentElement( 'info', 'linkType' ) && dialog.getValueOf( 'info', 'linkType' ) != 'local' )
								return true;

							if ( !( /[-a-zA-Z0-9_]+/ ).test( this.getValue() ) ) {
								alert( commonLang.invalidValue ); // jshint ignore:line
								return false;
							}
						...
						setup: function( data ) {
							if ( data.localurl )
								this.setValue( data.localurl );
							var inputId = this._.inputId;
							$.get(editor.config.linklocal_autocomplete, function(data) {
								$('#' + inputId).autocomplete({ source: data });
								$('#' + inputId).autocomplete('widget').css('z-index', 11001);
                            });
						},
						commit: function( data ) {
							data.localurl = this.getValue();
						}
					} ],
					setup: function() {
						if ( !this.getDialog().getContentElement( 'info', 'linkType' ) )
