...
( function() {
	CKEDITOR.plugins.add( 'linklocal', {
		...
		init: function( editor ) {
			...
			var prefixRegex = editor.config.linklocal_prefix;
			prefixRegex = prefixRegex.replace(/[-[\]{}()*+?.,\\^$|#\s]/g, "\\$&");
			prefixRegex = new RegExp('^'+  prefixRegex + '([-a-zA-Z0-9_]+)$');
			editor.config.locallink_prefix_regex = prefixRegex;
			...
			if ( editor.ui.addButton ) {
				editor.ui.addButton( 'Link', {
					label: editor.lang.linklocal.toolbar,
					command: 'link',
					toolbar: 'links,10'
				} );
				editor.ui.addButton( 'Unlink', {
					label: editor.lang.linklocal.unlink,
					command: 'unlink',
					toolbar: 'links,20'
				} );
				editor.ui.addButton( 'Anchor', {
					label: editor.lang.linklocal.anchor.toolbar,
					command: 'anchor',
					toolbar: 'links,30'
				} );
			}
			...
			editor.on( 'doubleclick', function( evt ) {
				var element = CKEDITOR.plugins.linklocal.getSelectedLink( editor ) || evt.data.element;
				...
				if ( !element.isReadOnly() ) {
					...
					} else if ( CKEDITOR.plugins.linklocal.tryRestoreFakeAnchor( editor, element ) ) {
			...
			if ( editor.addMenuItems ) {
				editor.addMenuItems( {
					anchor: {
						label: editor.lang.linklocal.anchor.menu,
						command: 'anchor',
						group: 'anchor',
						order: 1
					},
					...
					removeAnchor: {
						label: editor.lang.linklocal.anchor.remove,
					...
					link: {
						label: editor.lang.linklocal.menu,
						command: 'link',
						group: 'link',
						order: 1
					},
			...
			if ( editor.contextMenu ) {
				editor.contextMenu.addListener( function( element ) {
					...
					var anchor = CKEDITOR.plugins.linklocal.tryRestoreFakeAnchor( editor, element );

					if ( !anchor && !( anchor = CKEDITOR.plugins.linklocal.getSelectedLink( editor ) ) )
						return null;
		...
		afterInit: function( editor ) {
			...
			if ( pathFilters ) {
				pathFilters.push( function( element, name ) {
					if ( name == 'a' ) {
						if ( CKEDITOR.plugins.linklocal.tryRestoreFakeAnchor( editor, element ) || ( element.getAttribute( 'name' ) && ( !element.getAttribute( 'href' ) || !element.getChildCount() ) ) )
							return 'anchor';
					}
				} );
			}
	...
	function protectEmailLinkAsFunction( editor, email ) {
		var plugin = editor.plugins.linklocal,
	 ...
	 * @singleton
	 */
	CKEDITOR.plugins.linklocal = {
		/**
		 * Get the surrounding link element of the current selection.
		 *
		 *		CKEDITOR.plugins.linklocal.getSelectedLink( editor );
		 *
		...
		parseLinkAttributes: function( editor, element ) {
			var href = ( element && ( element.data( 'cke-saved-href' ) || element.getAttribute( 'href' ) ) ) || '',
				compiledProtectionFunction = editor.plugins.linklocal.compiledProtectionFunction,
				emailProtection = editor.config.emailProtection,
				javascriptMatch, emailMatch, anchorMatch, urlMatch, localMatch, articleMatch,
				retval = {};
			...
			if ( !retval.type ) {
				...
				else if ( ( articleMatch = href.match( editor.config.locallink_prefix_regex ) ) ) {
					retval.type = 'local';
					retval.localurl = articleMatch[ 1 ];
				}
				else if ( ( localMatch = href.match( localRegex ) ) ) {
					retval.type = 'url';
					retval.url = {};
					retval.url.protocol = '/';
					retval.url.url = localMatch[ 1 ];
				}
		...
		getLinkAttributes: function( editor, data ) {
			...
			switch ( data.type ) {
				case 'url':
					...
					break;
				case 'local':
					var url = CKEDITOR.tools.trim( data.localurl ) || '';

					set[ 'data-cke-saved-href' ] = editor.config.linklocal_prefix + url;
	...
	CKEDITOR.removeAnchorCommand.prototype = {
		exec: function( editor ) {
			var sel = editor.getSelection(),
				...
				anchor;
			if ( sel && ( anchor = sel.getSelectedElement() ) && ( !anchor.getChildCount() ? CKEDITOR.plugins.linklocal.tryRestoreFakeAnchor( editor, anchor ) : anchor.is( 'a' ) ) )
				anchor.remove( 1 );
			else {
				if ( ( anchor = CKEDITOR.plugins.linklocal.getSelectedLink( editor ) ) ) {
